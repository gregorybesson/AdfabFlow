...
class Domain extends EventProvider implements ServiceManagerAwareInterface
{
    ...
    {
    	$domain  = new \AdfabFlow\Entity\OpenGraphDomain();
    	$form  = $this->getServiceManager()->get('adfabflow_domain_form');
    	$form->bind($domain);
    	$form->setData($data);
    ...
    {
    	$mapping  = new \AdfabFlow\Entity\OpenGraphStoryMapping();
    	$form  = $this->getServiceManager()->get('adfabflow_storymapping_form');
    	$form->bind($mapping);
    	$form->setData($data);
    	...
    	foreach($objectAttributes as $attribute){
    		...
    		if(! in_array($attribute->getCode(), $existingMappingArray)){
    			$attributeMapping = new \AdfabFlow\Entity\OpenGraphObjectAttributeMapping();
    ...
    {
    	$form  = $this->getServiceManager()->get('adfabflow_objectattributemapping_form');
    ...
    public function getDomainMapper()
    {
        if (null === $this->domainMapper) {
            $this->domainMapper = $this->getServiceManager()->get('adfabflow_domain_mapper');
        }
    ...
    public function getStoryMappingMapper()
    {
    	if (null === $this->storyMappingMapper) {
    		$this->storyMappingMapper = $this->getServiceManager()->get('adfabflow_storyMapping_mapper');
    	}
    ...
    public function getObjectAttributeMappingMapper()
    {
    	if (null === $this->objectAttributeMappingMapper) {
    		$this->objectAttributeMappingMapper = $this->getServiceManager()->get('adfabflow_objectattributemapping_mapper');
    	}
