...
class Event extends EventProvider implements ServiceManagerAwareInterface
{
    ...
    {
        $em = $this->getServiceManager()->get('adfabflow_doctrine_em');
        ...
        $query = $em->createQuery('SELECT ' . $aggregate . ' FROM AdfabFlow\Entity\Event e WHERE e.user = :user AND e.actionId in (?1)');
    ...
    public function getEventMapper()
    {
        if (null === $this->eventMapper) {
            $this->eventMapper = $this->getServiceManager()->get('adfabflow_event_mapper');
        }
