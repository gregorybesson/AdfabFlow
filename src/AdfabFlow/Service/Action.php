...
class Action extends EventProvider implements ServiceManagerAwareInterface
{
    ...
    {
    	$action  = new \AdfabFlow\Entity\OpenGraphAction();
    	$form  = $this->getServiceManager()->get('adfabflow_action_form');
    	$form->bind($action);
    	$form->setData($data);
    ...
    public function getActionMapper()
    {
        if (null === $this->actionMapper) {
            $this->actionMapper = $this->getServiceManager()->get('adfabflow_action_mapper');
        }
