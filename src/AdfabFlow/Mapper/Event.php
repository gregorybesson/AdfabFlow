...
class Event extends AbstractDbMapper implements ActionInterface
{
    ...
    public function findActivity($user, $type='')
    {
        ...
        $query = $this->em->createQuery('SELECT e FROM AdfabFlow\Entity\Event e WHERE e.user = :user AND e.points > 0 and e.actionId in (?1) ORDER BY e.createdAt DESC');
    ...
    {
        if (null === $this->er) {
            $this->er = $this->em->getRepository('AdfabFlow\Entity\Event');
        }
