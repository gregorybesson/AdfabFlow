...
namespace AdfabFlow\Controller\Admin;

use Zend\Paginator\Paginator;
use Zend\Mvc\Controller\AbstractActionController;
use Zend\View\Model\ViewModel;
use AdfabFlow\Entity\OpenGraphDomain as Domain;
use AdfabFlow\Entity\OpenGraphStoryMapping as Mapping;
...
class DomainController extends AbstractActionController
{
	...
	public function createAction()
	{
		...
		$viewModel->setTemplate('adfab-flow/admin/domain/domain');
	...
	public function removeAction()
	{
		...
		if ($domain) {
			try {
				...
				$this->flashMessenger()->setNamespace('adfabflow')->addMessage('The domain has been removed');
			} catch (\Doctrine\DBAL\DBALException $e) {
				$this->flashMessenger()->setNamespace('adfabflow')->addMessage('This domain is included in a domain and can\'t be removed');
				//throw $e;
			}
		}
	...
	public function createStoryAction()
	{
		$service 	= $this->getAdminDomainService();
		$viewModel = new ViewModel();
		$viewModel->setTemplate('adfab-flow/admin/domain/story');
		$domainId = $this->getEvent()->getRouteMatch()->getParam('domainId');
	...
	public function removeStoryAction()
	{
		...
		if ($storyMapping) {
			try {
				...
				$this->flashMessenger()->setNamespace('adfabflow')->addMessage('The story has been removed');
			} catch (\Doctrine\DBAL\DBALException $e) {
				$this->flashMessenger()->setNamespace('adfabflow')->addMessage('This story is included in a story and can\'t be removed');
				//throw $e;
			}
		}
	...
	/*public function createAttributeAction()
	{
		...
		$viewModel = new ViewModel();
		$viewModel->setTemplate('adfab-flow/admin/object/attribute');
	...
	public function editAttributeAction()
	{
		...
		$viewModel = new ViewModel();
		$viewModel->setTemplate('adfab-flow/admin/domain/attribute');
	...
	/*public function removeAttributeAction()
	{
		...
		if ($attribute) {
			try {
				...
				$this->flashMessenger()->setNamespace('adfabflow')->addMessage('The attribute has been removed');
			} catch (\Doctrine\DBAL\DBALException $e) {
				$this->flashMessenger()->setNamespace('adfabflow')->addMessage('This attribute is included in a story and can\'t be removed');
				//throw $e;
			}
		}
	...
	public function getAdminDomainService()
	{
		if (!$this->adminDomainService) {
			$this->adminDomainService = $this->getServiceLocator()->get('adfabflow_domain_service');
		}
